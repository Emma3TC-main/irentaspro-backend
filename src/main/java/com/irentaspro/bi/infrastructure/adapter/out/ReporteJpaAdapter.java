...
public class ReporteJpaAdapter implements ReporteRepository {
        ...
        public List<KPIContrato> obtenerKPIContratos() {
                ...
                var query = em.createQuery("""
                                SELECT new com.irentaspro.bi.domain.model.KPIContrato(
                                    c.id,
                                    c.monto,
                                    c.montoPendiente,
                                    c.inicio,
        ...
        public KPIGlobal obtenerKPIGeneral() {
                ...
                long totalContratos = em
                                .createQuery("SELECT COUNT(c) FROM ContratoJpaEntity c", Long.class)
                                .getSingleResult();

                long contratosActivos = em
                                .createQuery("SELECT COUNT(c) FROM ContratoJpaEntity c WHERE c.estado = 'ACTIVO'",
                                                Long.class)
                                .getSingleResult();
                ...
                long pagosRealizados = em
                                .createQuery("SELECT COUNT(p) FROM PagoEntity p WHERE p.estado = 'COMPLETADO'",
                                                Long.class)
                                .getSingleResult();

                BigDecimal deudaPendiente = Optional.ofNullable(
                                em.createQuery("SELECT SUM(c.montoPendiente) FROM ContratoJpaEntity c",
                                                BigDecimal.class)
                                                .getSingleResult())
                                .orElse(BigDecimal.ZERO);
